            ...
            tools: [
                {
                    ...
                    description: 'Send a message, request, or handoff to another AI desktop agent (cursor, antigravity, claude, codex, or all).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            from_agent: {
                            ...
                            relay_parent_id: {
                                type: 'integer',
                                description: 'ID of the claimed parent message when relaying. The server derives and enforces origin and hop count.',
                            },
                        },
                ...
                {
                    name: 'fetch_inbox',
                    description: 'Fetch unread or pending messages/handoffs addressed to the calling agent.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            agent_id: {
                ...
                },
                {
                    ...
                    description: 'Update the status of a received message (e.g. mark as COMPLETED). Prefer claim_message to take ownership.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            message_id: {
                ...
                {
                    name: 'claim_message',
                    description: 'Atomically claim a message addressed to you. Sets claimed_by, marks READ. Returns claimed:false if another agent already claimed it.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                ...
                {
                    name: 'claim_task',
                    description: 'Atomically claim a TODO task assigned to you or unassigned. Sets IN_PROGRESS. Returns claimed:false if another agent already claimed it.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                ...
                },
                {
                    ...
                    description: 'Save key context, architectural decisions, code changes, or lessons learned to the persistent shared database.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            agent_id: {
                ...
                },
                {
                    ...
                    description: 'Query shared memory stored across all desktop agents (Cursor, Antigravity, Claude, Codex).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            query: {
        ...
        try {
            switch (name) {
                case 'send_agent_message': {
                    ...
                    const status = toolArgs.status || 'UNREAD';
                    const relayParentId = typeof toolArgs.relay_parent_id === 'number'
                        ? toolArgs.relay_parent_id
                        : undefined;
                    const msg = await messagingService.sendMessage(fromAgent, toAgent, topic, content, status, relayParentId);
                    return {
                        content: [
                            {
                                type: 'text',
                ...
                case 'fetch_inbox': {
                    ...
                    const status = toolArgs.status ? toolArgs.status : undefined;
                ...
                case 'create_task': {
                    ...
                    const status = toolArgs.status ? toolArgs.status : 'TODO';
