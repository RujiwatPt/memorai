...
export class Database {
    ...
    async migrate() {
        ...
        try {
            ...
            try {
                const messageCols = await this.all(`PRAGMA table_info(messages)`);
                ...
                if (!messageNames.has('relay_hop')) {
                    await this.exec(`ALTER TABLE messages ADD COLUMN relay_hop INTEGER NOT NULL DEFAULT 1`);
                }
                if (!messageNames.has('relay_parent_id')) {
                    await this.exec(`ALTER TABLE messages ADD COLUMN relay_parent_id INTEGER`);
                }
                await this.exec(`UPDATE messages SET relay_origin = from_agent WHERE relay_origin IS NULL`);
                const taskCols = await this.all(`PRAGMA table_info(tasks)`);
